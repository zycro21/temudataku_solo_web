...
export interface ArticleRichTextEditorRef {
...
interface ArticleRichTextEditorProps {
  ...
  onFocus?: () => void;
  onBlur?: () => void;
  onMount?: (ref: ArticleRichTextEditorRef) => void;
  onUnmount?: () => void;
...
const ArticleRichTextEditor = forwardRef<
...
>(
  (
    ...
    // 🔥 BARU — lacak HTML terakhir yang KITA SENDIRI kirim ke parent lewat
    // onChange. Dipakai buat bedain: value prop berubah karena efek balik
    // dari ngetik sendiri (sudah sama persis isi DOM-nya, nggak perlu
    // di-apply ulang) vs value berubah dari LUAR — undo/redo, atau ganti
    // item lain — yang WAJIB di-sync ulang ke innerHTML.
    ...
    }, [fireSelectionChange, emitChange]);
    ...
    const buildAPI = useCallback(
      ...
      [fireSelectionChange, toggleItalic, toggleStrikethrough, emitChange],
    );
    ...
    // 🔥 BARU — sync ulang innerHTML kalau `value` berubah dari LUAR
    // (undo/redo, atau parent nge-reset state canvasItems) SETELAH mount.
    // Effect mount di atas cuma inject sekali; tanpa ini, undo/redo di
    // parent nggak pernah kelihatan di layar meskipun state-nya sudah
    // benar berubah — karena contentEditable ini "uncontrolled" (sengaja,
    // biar kursor nggak reset tiap ngetik). Dibedain dari echo hasil
    // ngetik sendiri lewat lastEmittedValueRef: kalau value yang masuk
    // SAMA PERSIS sama yang barusan KITA emit sendiri, DOM udah benar,
    // nggak perlu disentuh (biar kursor nggak lompat).
    useEffect(() => {
...
ArticleRichTextEditor.displayName = "ArticleRichTextEditor";
