...
import type { StreamReviewListItem } from "@/app/admin/elearning/reviews/page";

interface Props {
  streams: StreamReviewListItem[];
  isLoading: boolean;
}

type SortField = "title" | "coursesCount" | "reviewCount" | "averageRating";
type SortDir = "asc" | "desc";

const LIMIT_OPTIONS = [5, 10, 20, 50];

export default function ReviewStreamListTable({ streams, isLoading }: Props) {
  ...
  // ── Search + sort dikerjakan di client (lihat catatan di page.tsx) ───────
  const filtered = useMemo(() => {
    ...
    return sorted;
  }, [streams, search, sortField, sortDir]);
    ...
    <div className="rounded-2xl border border-gray-200 bg-white shadow-sm mb-15">
      ...
      <div className="flex flex-col gap-3 border-b border-gray-100 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          <h2 className="text-base font-bold text-gray-900">Daftar Stream</h2>
              ...
              <tr>
                <td colSpan={6} className="px-5 py-14 text-center">
                  <div className="flex flex-col items-center gap-2 text-gray-400">
                    <Inbox size={32} />
                    <p className="text-sm">Belum ada stream ditemukan.</p>
                  </div>
                </td>
              </tr>
            )}
              ...
              paginated.map((s) => (
                <tr key={s.id} className="transition hover:bg-gray-50/80">
                  ...
                  <td className="px-5 py-4 text-right">
                    <Link
                      href={{
                        pathname: `/admin/elearning/reviews/${s.id}`,
                        query: { title: s.title },
                      }}
      ...
      {totalPages > 1 && (
        <div className="flex flex-col gap-3 border-t border-gray-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-gray-500">
            ...
            {Math.min(currentPage * limit, total)} dari {total} stream
          </p>
