...
export default function CoursesTable({
  ...
  const fetchHistory = async (courseId: string, page: number) => {
    ...
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/elearningSubChapter/subchapters/${courseId}/history`,
        {
  ...
  useEffect(() => {
    if (historyModal) {
      fetchHistory(historyModal.courseId, historyPage);
    }
    setSelectedLog(null);
  ...
  const openHistoryModal = (course: any) => {
    setHistoryPage(1);
    setHistoryModal({ courseId: course.id, courseTitle: course.name });
                    ...
                    {openMenu === course.id && (
                      <div
                        ...
                        className={`absolute right-0 w-40 bg-white border rounded-lg shadow-md z-50 text-sm ${
                          menuOpenUpward ? "bottom-full mb-2" : "top-full mt-2"
                        }`}
                      >
                        <button
                          onClick={(e) => {
          ...
          >
            ...
            <div className="overflow-y-auto flex-1 px-7 py-5">
              {historyLoading ? (
                ...
                <p className="text-sm text-gray-400 text-center py-10">
                  Belum ada riwayat perubahan untuk course ini.
                </p>
              ) : (
                        ...
                        <div key={row.key}>
                          ...
                          <div className="mt-1.5 space-y-1.5">
                            ...
                            <div className="rounded-lg bg-emerald-50 border border-emerald-100 px-3 py-2">
                              ...
                              row.newVal ? (
                                <img
                                  src={`${process.env.NEXT_PUBLIC_API_BASE_URL}${row.newVal}`}
                                  alt="cover"
                                  className="w-24 h-16 object-cover rounded-md border border-emerald-200 mt-1"
                                  onError={(e) => {
                                    (
