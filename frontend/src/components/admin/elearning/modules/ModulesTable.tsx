...
export default function ModulesTable({
  ...
  const fetchHistory = async (moduleId: string, page: number) => {
    setHistoryLoading(true);
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/elearningSubBab/subbabs/${moduleId}/history`,
        {
  ...
  useEffect(() => {
    if (historyModal) {
      fetchHistory(historyModal.moduleId, historyPage);
    }
    setSelectedLog(null);
  ...
  const openHistoryModal = (mod: SubBab) => {
    setHistoryPage(1);
    setHistoryModal({ moduleId: mod.id, moduleTitle: mod.title });
          ...
          >
            ...
            <div className="overflow-y-auto flex-1 px-7 py-5">
              {historyLoading ? (
                ...
                <p className="text-sm text-gray-400 text-center py-10">
                  Belum ada riwayat perubahan untuk module ini.
                </p>
              ) : (
