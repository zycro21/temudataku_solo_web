  ...
  return (
    <section className="py-20 px-4 bg-gradient-to-b from-white to-emerald-50">
      <div className="max-w-5xl mx-auto text-center">
        ...
        <p className="text-gray-500 font-medium text-sm max-w-xl mx-auto mb-12 leading-relaxed">
        ...
        <div className="flex justify-center gap-6 mb-6 flex-wrap">
          {firstRow.map((c, i) => (
            <div
              key={i}
              className="bg-white border border-emerald-100 rounded-xl px-7 py-5 h-20 flex items-center justify-center shadow-sm hover:shadow-md hover:-translate-y-0.5 transition"
            >
              <img
                src={c.logo}
                alt={c.name}
                className="max-h-10 max-w-[140px] object-contain grayscale opacity-80 hover:grayscale-0 hover:opacity-100 transition"
              />
            </div>
          ))}
        </div>
