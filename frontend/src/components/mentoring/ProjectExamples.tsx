...
const projects = [
  {
    ...
    description: "Project ini menganalisis hubungan angka stunting dengan faktor ekonomi di Papua Barat, dan menemukan korelasi positif di antara keduanya.",
  ...
  },
  {
    ...
    description: "Penelitian mendalam tentang pertumbuhan sektor ekonomi digital di Indonesia dan dampaknya terhadap UMKM.",
    ...
    <section className="py-8 md:py-16 bg-white">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8 md:mb-16">
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-4 md:mb-6 px-4">Project Sejauh Ini, Keren Nggak Sih?</h2>
          <p className="text-base md:text-lg text-gray-700 max-w-4xl mx-auto leading-relaxed px-4">
            Bukan sekadar latihan—ini hasil nyata dari mereka yang udah serius ngejalanin challenge di TemuDataku. Dari analisis data sampai visualisasi yang insight-driven, semua project ini nunjukin gimana latihan bisa beneran ngasah
            skill. Siap-siap terinspirasi!
          </p>
        </div>
          ...
          >
            <CarouselContent className="py-2 md:py-5 -ml-2 md:-ml-4">
              {projects.map((project) => (
                <CarouselItem key={project.id} className="pl-2 md:pl-4 basis-[85%] sm:basis-[75%] md:basis-1/2 lg:basis-[45%]">
                  <div className="bg-bg-color-primary rounded-lg md:rounded-xl overflow-hidden h-full flex flex-col">
                    ...
                    <div className="p-4 md:p-6 flex flex-col flex-1">
                      <h3 className="text-lg md:text-xl font-bold text-gray-900 mb-3 md:mb-4 leading-tight line-clamp-3">{project.title}</h3>

                      <p className="text-sm md:text-base text-gray-600 mb-4 md:mb-6 leading-relaxed line-clamp-3 flex-1">{project.description}</p>

                      {/* Authors */}
                      <div className="flex items-center mb-4 md:mb-6 gap-3">
                        <UserRound className="w-4 h-4" />

                        <span className="text-xs md:text-sm text-gray-700 line-clamp-1">{project.authors.join(", ")}</span>
                      </div>

                      {/* Button */}
                      <Button className="w-fit bg-emerald-500 hover:bg-emerald-600 text-white px-4 md:px-6 py-2 rounded-lg font-medium text-sm md:text-base">Jelajahi Proyek</Button>
                    </div>
                  </div>
                </CarouselItem>
              ))}
