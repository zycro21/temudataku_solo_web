...
export default function MentoringDetailModal({
  ...
  function parseDbDate(dateStr: string): Date {
    // dateStr contoh: "03-10-2025"
    const [day, month, year] = dateStr.split("-").map(Number);
    return new Date(year, month - 1, day); // month - 1 karena JS bulan mulai dari 0
  }
      ...
      >
        {!event ? (
          <>
            <DialogHeader>
              <DialogTitle>
                <VisuallyHidden>Loading detail mentoring</VisuallyHidden>
              </DialogTitle>
            </DialogHeader>
            ...
            <DialogHeader className="flex flex-row items-center justify-between border-b px-6 py-4">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Detail jadwal {typeLabels[event.type]} Anda.
                </p>
              </div>
            </DialogHeader>
                  ...
                  <div className="pl-5 space-y-3">
                    {event.mentees.map((mentee, idx) => (
                      <div key={idx} className="flex items-center gap-3">
                        <Image
                          src={
                            mentee.avatar || "/assets/dashboard/user/avatar.png"
                          }
                          alt={mentee.name}
                          width={32}
                          height={32}
