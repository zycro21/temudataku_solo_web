...
export default function EditReviewModal({
  ...
  // Prefill data dari API saat modal terbuka
  useEffect(() => {
    if (open && project?.id) {
  ...
  const isStep3Valid =
    ...
    answers["generalComment"] !== undefined &&
    answers["improvement"] !== undefined &&
    answers["revision"] !== undefined &&
    (answers["revision"] === "no" || answers["revisionDate"]);
  ...
  const handleSubmitEditReview = async () => {
    ...
    try {
      ...
      toast.success("Perubahan review berhasil disimpan!");
      setShowSuccessModal(true);
    } catch (error: any) {
      console.error(error);
      toast.error(
        error.response?.data?.message || "Gagal menyimpan perubahan review."
      );
    }
  };
    ...
    <>
      ...
      <ConfirmEditModal
        ...
        onConfirm={() => {
          ...
          handleSubmitEditReview(); // panggil API di sini
        }}
      />
