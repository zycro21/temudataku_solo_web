...
// ============================================================
// TAB ORDER: E-LEARNING, BOOTCAMP, MENTORING, AYCL
// ============================================================
const tabs = [
...
// ============================================================
// E-LEARNING SUBSCRIPTION HELPER
// ============================================================
...
function PlanSkeleton() {
  return (
    <div className="mx-auto w-full max-w-[280px] rounded-xl overflow-hidden shadow-lg animate-pulse">
...
const LearningPathsSection = forwardRef<HTMLDivElement>((props, ref) => {
  ...
  // ============================================================
  // RENDER E-LEARNING PLANS (DIPERBAIKI - TAMBAH MIN-HEIGHT)
  // ============================================================
      ...
      return (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <h3 className="text-xl font-bold text-gray-900 mb-2">
            Paket E-Learning Akan Segera Hadir
          </h3>
          <p className="text-sm text-gray-600 max-w-md">
            Paket langganan sedang dalam persiapan. Pantau terus untuk informasi
            terbaru!
          </p>
        </div>
      );
    ...
    return (
      <div className="grid md:grid-cols-3 gap-4 justify-items-center items-start">
        {sortedPlans.map((plan) => {
          ...
          const strikePrice = Math.round(originalPrice * 1.2);
          const perUnitPrice = Math.round(originalPrice / months);
          ...
          return (
            ...
            >
              {/* TOP - BERWARNA */}
              <div
                className={`relative py-3 pl-4 text-white ${
                  isPopuler ? "bg-emerald-500" : "bg-[#1f3b73]"
                }`}
              >
                {isPopuler && (
                  <div className="absolute top-0 left-0 right-0 bg-[#F49D07] text-gray-900 text-xs font-bold py-1.5 text-center tracking-wide">
                    TERPOPULER!
                  </div>
                )}

                <h3 className={`text-lg font-bold ${isPopuler ? "mt-8" : ""}`}>
                  {months} Bulan
                </h3>
                <p className="text-[10px] opacity-90 mt-1 mb-4">
                  PAKET PEMBELAJARAN E-LEARNING
                </p>

                {meta.discountAmount > 0 && (
                  <p className="text-xs line-through opacity-80 mb-1">
                ...
                <p className="text-[10px] opacity-90 leading-relaxed">
                  Setara {formatRupiah(perUnitPrice)} / bulan
                </p>
              </div>

              {/* BOTTOM - PUTIH - PAKAI FLEX COLUMN + MIN-HEIGHT */}
              <div className="bg-white p-5 flex flex-col h-full min-h-[280px]">
                {/* Features - TAMPILKAN SEMUA */}
                ...
                {/* BUTTON - PAKAI MT-AUTO AGAR DI BAWAH */}
                <Button
                  onClick={() => handleSubscribe(plan.id)}
                  disabled={isSubmitting || submittingPlanId !== null}
                  className={`mt-auto w-full text-sm py-2.5 transition-all duration-300 shadow-md hover:shadow-lg hover:-translate-y-[2px] disabled:opacity-60 disabled:hover:translate-y-0 ${
                    isPopuler
                      ? "bg-emerald-500 text-white hover:bg-emerald-600"
                      : "bg-[#1f3b73] text-white hover:bg-[#162d5a]"
                  }`}
                >
                  {isSubmitting ? (
                ...
                {/* PROGRESS BAR - hanya untuk yang populer */}
                {isPopuler && (
                  <div className="mt-3">
                    <p className="text-xs font-semibold text-red-500 mb-1 text-center">
                      🔥 Segera Habis!
                    </p>
                    <div className="w-full h-1.5 rounded-full bg-gray-100 overflow-hidden">
                      <div className="h-full rounded-full bg-gradient-to-r from-emerald-400 to-emerald-600 w-[70%]" />
                    </div>
            ...
            >
              ...
              <CardContent className="p-5">
                ...
                <div className="space-y-2">
                  <Button
                    onClick={() => router.push(`/programs/${bootcamp.slug}`)}
    ...
    return (
      <div className="grid md:grid-cols-2 gap-4 justify-items-center max-w-3xl mx-auto">
        {mentoringOptions.map((option) => (
          <Card
            key={option.id}
            className="overflow-hidden shadow-md border-0 w-full max-w-sm"
          >
            <div className="relative">
              <Image
                src={option.image}
      ...
      return (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-sm text-gray-500 max-w-sm">
            Program batch berikutnya sedang dalam persiapan. Pantau terus untuk
        ...
        {ayclBatches.map((batch, index) => (
          <motion.div
            ...
            className={`relative overflow-hidden rounded-2xl 
              p-6 sm:p-8 md:p-10 text-white shadow-xl
              ${
                index === 1
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-8">
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <div className="flex items-baseline gap-2 mb-6 justify-center md:justify-start">
                  <span className="text-emerald-200 text-base">Hanya</span>
                ...
                <div className="flex flex-wrap gap-3 justify-center md:justify-start">
                  <Button
                    onClick={() => router.push(`/aycl?slug=${batch.slug}`)}
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "elearning":
        return renderElearning();
      case "program":
        return renderBootcamp();
      case "mentoring":
        return renderMentoring();
      case "aycl":
        return renderAycl();
      default:
        return null;
